...
* --- 全局设置 ---
global PROJECT_ROOT "{{PROJECT_PATH}}"
...
* --- S1: 自动检测 Excel sheet ---
local excel_file "$RAW_DATA/{{DATA_FILENAME}}"
capture {
...
* --- S4: 手动缩尾 (零外部依赖) ---
local winsor_vars "{{Y_VARNAME}} {{D_VARNAME}} {{M1_VARNAME}} {{M2_VARNAME}} {{M3_VARNAME}}"
* 补充：自动检测其他连续变量
foreach var of varlist _all {
    capture confirm numeric variable `var'
    if _rc == 0 {
        ...
        if r(r) > 100 & strpos(lower("`var'"), "{{YEAR_VARNAME}}") == 0 & strpos(lower("`var'"), "{{ID_VARNAME}}") == 0 {
            local already = 0
...
* --- S5: 面板设定 ---
local id_var "{{ID_VARNAME}}"
local year_var "{{YEAR_VARNAME}}"
if "`id_var'" != "" & "`year_var'" != "" {
